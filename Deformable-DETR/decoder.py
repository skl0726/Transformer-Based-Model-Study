...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, C, M, K, n_levels, last_feat_height, last_feat_width, d_ffn=1024, dropout=0.1, normalize_before=False):
        ...
        Args: 
            ...
            - d_ffn: feed forwara network dimension
            - n_levels: multiscale parameter
            - M: number of attention heads
            - K: nubmer of sampling points
        """
        super().__init__()
        # Deformable Attention part
        self.def_attn = DeformableHeadAttention(last_height=last_feat_height,
                                                last_width=last_feat_width,
                                                C=C, M=M, K=K, L=n_levels,
                                                dropout=dropout,
    ...
    def forward(self, query_objects, out_encoder, ref_points, tgt_mask=None, memory_masks=None,
                tgt_key_padding_mask=None, memory_key_padding_masks=None, positional_embeddings=None, query_poses=None):
        """
        Args:
        ...
        if self.normalize_before:
            return self._forward_pre_norm(query_objects, out_encoder, ref_points,
                                          tgt_mask, memory_masks, tgt_key_padding_mask, memory_key_padding_masks,
                                          positional_embeddings, query_poses)
        return self._forward_post_norm(query_objects, out_encoder, ref_points,
                                       tgt_mask, memory_masks, tgt_key_padding_mask, memory_key_padding_masks,
                                       positional_embeddings, query_poses)
    
    def _forward_pre_norm(self, query_objects, out_encoder, ref_points,
                          tgt_mask=None, memory_masks=None, tgt_key_padding_mask=None, memory_key_padding_masks=None,
                          positional_embeddings=None, query_poses=None):
        
        query_objects_2 = self.norm1(query_objects) # pre normalization
        ...
        query_objects = query_objects + self.dropout1(query_objects_2)
        ...
        # query_obejcts is of same shape as nn.Embedding(number of object queries, C)
        query_objects = query_objects.transpose(0, 1).unsqueeze(dim=2) # L, B, C -> B, L, 1, C (L: number of object queries, B: size of batch)
        ref_points = ref_points.transpose(0, 1).unsqueeze(dim=2) # L, B, C -> B, L, 1, C (L: number of object queries, B: size of batch)
        ...
        query_objects = query_objects.transpose(0, 1).contiguous() # B, L, 1, C -> L, B, C

        return query_objects

    def _forward_post_norm(self, query_objects, out_encoder, ref_points,
                          tgt_mask=None, memory_masks=None, tgt_key_padding_mask=None, memory_key_padding_masks=None,
                          positional_embeddings=None, query_poses=None):
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, query_objects, out_encoder, ref_points,
                tgt_mask=None, memory_masks=None, tgt_key_padding_mask=None, memory_key_padding_masks=None,
                positional_embeddings=None, query_poses=None):
        ...
        for layer in self.layers:
            output = layer(output, out_encoder, ref_points,
                           tgt_mask=tgt_mask, memory_masks=memory_masks,
                           tgt_key_padding_mask=tgt_key_padding_mask, memory_key_padding_masks=memory_key_padding_masks,
                           positional_embeddings=positional_embeddings, query_poses=query_poses)
