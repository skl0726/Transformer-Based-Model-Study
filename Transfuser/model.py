...
class LidarCenterNetHead(BaseDenseHead, BBoxTestMixin):
    ...
    Args:
        ...
        - loss_wh (dict | None): config of wh loss (default: L1 Loss)
        - loss_offset (dict | None): config of offset loss (default: L1 Loss)
        - train_cfg (dict | None): training config. useless in CenterNet, but keep this variable for SingleStageDetector (default: None)
    ...
    def forward(self, feats):
        ...
        Returns:
            - center_heatmap_preds (List[Tensor]): center predict heatmaps for all levels, the channel number is num_classes
            - wh_preds (List[Tensor]): wh predicts for all levels, the channel number is 2
            - offset_pres (List[Tensor]): offset predicts for all levels, the channel number is 2
        """
    ...
    def forward_single(self, feat):
        ...
        Returns:
            - center_heatmap_pred (Tensor): center predict heatmaps, the channel number is num_classes
            - wh_pred (Tensor): wh predicts, the channel number is 2
            - offset_pred (Tensor): offset predicts, the channel number is 2
        """
    ...
    def loss(self,
        ...
        # since the channel of wh_target and offset_target is 2, the avg_factor of loss_center_heatmap is always 1/2 of loss_wh and loss_offset.
    ...
    def get_targets(self, gt_bboxes, gt_labels, gt_ignores, feat_shape):
        ...
        Returns:
            - tuple[dict,float]: The float value is mean avg_factor, the dict has components below:
    ...
    def get_bboxes(self,
        ...
        Returns:
            - list[tuple[Tensor, Tensor]]: each item in result_list is 2-tuple
                the first item is an (n, 5) tensor, where 5 represent (tl_x, tl_y, br_x, br_y, score) and the score between 0 and 1
                the shape of the second tensor in the tuple is (n,), and each element represents the class label of the corresponding box
        """
        assert len(center_heatmap_preds) == len(wh_preds) == len(offset_preds) == 1
...
class LidarCenterNet(nn.Module):
    ...
    def __init__(self, config, device, image_architecture='resnet34', lidar_architecture='resnet18', use_velocity=True):
        ...
        self._model = TransfuserBackbone(config, image_architecture, lidar_architecture, use_velocity).to(self.device)
        ...
        self.decoder = nn.GRUCell(input_size=4 if self.gru_concat_target_point else 2, # 2 represents x, y coordinate
                                  hidden_size=self.config.gru_hidden_size).to(device)
        
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.output = nn.Linear(self.config.gru_hidden_size, 3).to(device)
    ...
    def forward_gru(self, z, target_point):
        ...
        # pred the waypoints in the vehicle coordinate and convert it to lidar coordinate here because the GT waypoints is in lidar coordinate
    ...
    def control_pid(self, waypoints, velocity, is_stuck):
        """
        predict vehicle control with a PID controller
        ...
        # when training, transform the waypoints to lidar coordinate, so we need to change is back when control
        ...
        if is_stuck:
            desired_speed = np.array(self.config.default_speed) # default speed of 14.4km/h
        ...
        aim = np.degrees(np.arctan2(aim[1], aim[0])) / 90.0
        if speed < 0.01:
    ...
    def forward_ego(self, rgb, lidar_bev, target_point, target_point_image, ego_vel,
                    bev_points=None, cam_points=None, save_path=None, expert_waypoints=None,
        ...
        if debug and self.i % 2 == 0 and not save_path is None:
            ...
            self.visualize_model_io(save_path, self.i, self.config, rgb, lidar_bev, target_point, pred_wp,
                                    pred_bev, pred_semantic, pred_depth, bboxes, self.device, gt_bboxes=None, 
                                    expert_waypoints=expert_waypoints, stuck_detector=stuck_detector, forced_move=forced_move)

        return pred_wp, rotated_bboxes

    def forward(self, rgb, lidar_bev, ego_waypoint, target_point, target_point_image, ego_vel, bev,
                label, depth, semantic, num_points=None, save_path=None, bev_points=None, cam_points=None):
        loss = {}
        ...
        if ((self.config.debug == True) and (self.i % self.config.train_debug_save_freq == 0) and (save_path != None)):
            with torch.no_grad():
                ...
                bboxes = bboxes[bboxes[:, -1] > self.config.bb_confidence_threshold]
                self.visualize_model_io(save_path, self.i, self.config, rgb, lidar_bev, target_point, pred_wp,
                                        pred_bev, pred_semantic, pred_depth, bboxes, self.device, gt_bboxes=label,
