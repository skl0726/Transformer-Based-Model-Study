...
class TransfuserBackbone(nn.Module):
    ...
    def top_down(self, x):
        ...
        p4 = self.relu(self.up_conv5(self.upsample(x)))
        p3 = self.relu(self.up_conv4(self.upsample(x)))
        p2 = self.relu(self.up_conv3(self.upsample(x)))
    ...
    def forward(self, image, lidar, velocity):
        ...
        # Lidar fusion at (B, 576, 16, 16)
        image_embd_layer3 = self.avgpool_img(image_features)
        lidar_embd_layer3 = self.avgpool_lidar(lidar_features)
        image_features_layer3, lidar_features_layer3 = self.transformer2(image_embd_layer3, lidar_embd_layer3, velocity)
        ...
        # Lidar fusion at (B, 1512, 8, 8)
        image_embd_layer4 = self.avgpool_img(image_features)
        lidar_embd_layer4 = self.avgpool_lidar(lidar_features)
        image_features_layer4, lidar_features_layer4 = self.transformer2(image_embd_layer4, lidar_embd_layer4, velocity)
...
class SegDecoder(nn.Module):
    def __init__(self, config, latent_dim=512):
        ...
        self.deconv1 = nn.Sequential(
            nn.Conv2d(self.latent_dim, self.config.deconv_channel_num_1, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.Conv2d(self.config.deconv_channel_num_1, self.config.deconv_channel_num_2, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True)
        )
        self.deconv2 = nn.Sequential(
            nn.Conv2d(self.config.deconv_channel_num_2, self.config.deconv_channel_num_3, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.Conv2d(self.config.deconv_channel_num_3, self.config.deconv_channel_num_3, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True)
        )
        self.deconv3 = nn.Sequential(
            nn.Conv2d(self.config.deconv_channel_num_3, self.config.deconv_channel_num_3, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.Conv2d(self.config.deconv_channel_num_3, self.num_class, kernel_size=3, stride=1, padding=1)
        )
...
class DepthDecoder(nn.Module):
    def __init__(self, config, latent_dim=512):
        ...
        self.deconv3 = nn.Sequential(
            nn.Conv2d(self.config.deconv_channel_num_3, self.config.deconv_channel_num_3, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
            nn.Conv2d(self.config.deconv_channel_num_3, 1, kernel_size=3, stride=1, padding=1)
        )
...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_exp, n_layer,
        ...
        super().__init__()
        self.n_embd = n_embd
        self.seq_len = 1 # currently only support seq_len 1
